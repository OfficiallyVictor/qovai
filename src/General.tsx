...
export default function General() {
  return (
    <div className="">
      <div className=" text-2xl px-4 sm:text-3xl md:text-4xl lg:text-5xl font-bold uppercase text-center mt-32 flex flex-col gap-2 lg:gap-5">
        <p className=" font-bold">generate posts for your </p>
      ...
      <div className="flex flex-col -translate-x-2 sm:-translate-x-6  sm:flex-row items-center sm:items-stretch gap-5 mt-10 justify-center">
        <div className="flex translate-x-[61px] sm:translate-x-20">
          {[1, 2, 3, 4].map((el, key) => (
            <div
              className={`h-10 w-11 border-2 
              ${key === 0 && "z-0 bg-blue-500"}
              ${key === 1 && "-translate-x-[50px] z-10 bg-red-500"}
              ${key === 2 && "-translate-x-[60px] z-20 bg-green-500"}
              ${key === 3 && "-translate-x-[80px] z-30 bg-blue-500"}
